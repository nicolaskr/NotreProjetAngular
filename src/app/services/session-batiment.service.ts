...
export class SessionBatimentService {
  ...
  constructor(private http: HttpClient, private batimentService: BatimentService) { }
  ...
  public construire(session: Session, id: number): Observable<SessionBatiment>{
    this.initHeaders();
    return this.http.post<SessionBatiment>(this.url + '/construction/' + session.id?.partie?.id + '/' + session.id?.compte?.id + '/' + id, { headers: this.headers });
  }
  ...
  public ameliorer(id:number): Observable<SessionBatiment> {
    this.initHeaders();
    return this.http.put<SessionBatiment>(this.url + '/amelioration/' + id, { headers: this.headers });
  }
